...
from repr_partitions_cluster.src.hmeans.improvement_choice import (
    CallbackBestImprovement,
    CallbackFirstImprovement,
)
...
def generator(init: int = 0):
    L_NUM_CLUSTERS = [2**i for i in range(1, 8)]
    L_NUM_POINTS = np.concatenate(
        (np.arange(20, 91, 10), np.arange(100, 1001, 100)), axis=0
    )
    NUM_COORDINATES = 2
    np.random.seed(0)
    global_index = -1
            ...
            for init_type in ["random", "kmean+"]:
                for ImprClass in [CallbackBestImprovement, CallbackFirstImprovement]:
                    if ImprClass.__name__ == "CallbackFirstImprovement":
                        iteration_orders = [CURR(), BACK(), RANDOM()]
                    else:
                        ...
                        for _ in range(1000):
                            ...
                            if init_type == "kmean+":
                                points_assign, clust_coords = kmean(
                                    (points_coords),
                                    np.copy(points_assign),
                                    np.copy(clust_coords),
                                )

                            init_cost = cost(points_coords, points_assign, clust_coords)
                            global_index += 1
                            if global_index >= init:
                                # Important: make a copy of the parameters to avoid problems with multiprocessig
                                yield np.copy(points_coords), np.copy(
                                    points_assign
                                ), np.copy(
                                    clust_coords
                                ), init_cost, num_clusters, num_points, init_type, ImprClass, iteration_order, global_index


def wrapper(args):
    (
        *args_hmeans_non_result,
        init_cost,
        num_clusters,
        num_points,
        init_type,
...
if __name__ == "__main__":
    PATH_RES = Path("./data/algos_results.csv")
    df = pd.read_csv(PATH_RES, sep=",", header=0, index_col=False)
    max_index = df.index.max() if len(df) > 0 else 0
    if PATH_RES.absolute().exists():
        input("Backup file already exists. Press enter to continue.")
    ...
    results = list(p.imap(wrapper, generator(max_index + 1), chunksize=100))
    p.close()
    p.join()
