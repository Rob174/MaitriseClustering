...
from tensorflow.keras.layers import Lambda, Softmax, Conv2D, GlobalAveragePooling2D,Flatten,Dense,Dropout
from tensorflow.keras.applications import ResNet50,DenseNet121
from repr_partitions_cluster.src.HDF5Generator import HDF5Generator, HDF5GeneratorFilter, FilterMode
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "min_delta": 1e-3,
        "patience": 10,
        "network": "resnet50", # ["resnet50","densenet121","dense"]
        "grid_size": 128, # [64,128,256]
        "last_layers":"flatten_dense_drop", # ["flatten_dense_drop","flatten_dense","glob_avg"]
        "num_samples": 20000,
    ...
    wandb.init(
        ...
        name="test",#"Resnet - Base model",
        tags=["top_view_image"],
        save_code=True,
        entity="romo-1245"
    )
    path_caches = [root / "image_dataset" / f"dataset_ia_2_clusters_grid_{config['grid_size']}px_{dataset}.hdf5" for dataset in ["tr", "val"]]
    ds = {
    ...
    path_cache = root  / "image_dataset" / f"dataset_ia_2_clusters_grid_{config['grid_size']}px_val.hdf5"
    path_metadata = root / "dataset.hdf5"
    for filter_mode in FilterMode:
        ds["val_"+filter_mode.value] = tf.data.Dataset.from_generator(HDF5GeneratorFilter(
                    path_cache,path_metadata,filter_mode
                ),
            output_signature=(
                tf.TensorSpec(shape=(config["grid_size"], config["grid_size"], 2), dtype=tf.float32),  # type: ignore
                tf.TensorSpec(shape=(2,), dtype=tf.float32),  # type: ignore
    ...
    if config["last_layers"] == "flatten_dense":
        ...
        for i,filters in enumerate(config["denses"]):
            ...
            end_model_layers.append(Dense(filters,activation="relu" if i<len(config["denses"])-1 else "linear"))
    else:
        raise Exception
    base_network = None
    ...
    callbacks = [
        ...
        ConfusionMatrix(validation_data=ds["val"],classes_names=("BI","FI"),name="full_validation"),
        ConfusionMatrix(validation_data=ds["val_random"],classes_names=("BI","FI"),name="random_validation"),
        ConfusionMatrix(validation_data=ds["val_kmeans+"],classes_names=("BI","FI"),name="kmeans_validation"),
        Predictions(
    ...
    wandb.log({"val_final_loss":loss,"val_final_MAE":metrics[0],"val_final_accuracy":metrics[1]})
    [loss, *metrics] = model.evaluate(ds["val_random"])
    wandb.log({"val_random_final_loss":loss,"val_random_final_MAE":metrics[0],"val_random_final_accuracy":metrics[1]})
    [loss, *metrics] = model.evaluate(ds["val_kmeans+"])
    wandb.log({"val_kmeans+_final_loss":loss,"val_kmeans+_final_MAE":metrics[0],"val_kmeans+_final_accuracy":metrics[1]})
    wandb.finish()
