...
def kmean(points_coords: np.ndarray, points_assign: np.ndarray, clust_coords: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    empty_clusters = [i for i in range(
        len(clust_coords)) if i not in clusters_with_pts]
    if len(empty_clusters) > 0:
        ...
        sorted_coords = np.dstack(np.unravel_index(np.argsort(d_pts_clust.ravel()), (len(
            clust_coords), len(points_coords)))).reshape(-1, 2)  # [num_pts, 2] in increasing order of dist
